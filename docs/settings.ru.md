...
| `vp8.max_bytes_per_sec` | Потолок скорости на проводе, байт/сек | `1000000` |

`vp8.max_bytes_per_sec` ограничивает байтрейт, который пейсер льёт в видеотрек. Дефолт 1000000 (1 МБ/с) - целевая скорость из issue #107. Policer-колено у каждого SFU своё, поэтому если на этой скорости поток падает - снижай до стабильной; на здоровом пути можно поднять. `0` оставляет дефолт.
